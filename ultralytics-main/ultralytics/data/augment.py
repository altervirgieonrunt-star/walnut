...
# Added by integrate_copy_paste.py

try:
    from ultralytics.data.copy_paste_enhanced import CopyPasteEnhanced, InstancePool
    
    # 全局实例池（单例模式）
    _GLOBAL_INSTANCE_POOL = None
    
    def get_or_create_instance_pool(dataset, max_images=200, max_pool_size=1000):
        """
        获取或创建全局实例池
        ...
        Returns:
            InstancePool对象
        """
        global _GLOBAL_INSTANCE_POOL
        if _GLOBAL_INSTANCE_POOL is None or len(_GLOBAL_INSTANCE_POOL) == 0:
            from ultralytics.data.copy_paste_enhanced import build_instance_pool_from_dataset
            LOGGER.info("Building global instance pool for Copy-Paste augmentation...")
            _GLOBAL_INSTANCE_POOL = build_instance_pool_from_dataset(
                dataset, 
                max_images=max_images,
                max_pool_size=max_pool_size
            )
        return _GLOBAL_INSTANCE_POOL
    
    COPY_PASTE_ENHANCED_AVAILABLE = True
    LOGGER.info("✅ Enhanced Copy-Paste augmentation is available!")
    
except ImportError as e:
    COPY_PASTE_ENHANCED_AVAILABLE = False
    LOGGER.warning(f"⚠️  Enhanced Copy-Paste not available: {e}")
