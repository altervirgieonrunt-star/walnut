...
# Enhanced Copy-Paste (optional import)
try:
    from ultralytics.data.copy_paste_enhanced import CopyPasteEnhanced, build_instance_pool_from_dataset  # type: ignore
    _COPY_PASTE_ENHANCED_AVAILABLE = True
except Exception:  # pragma: no cover - optional feature
    _COPY_PASTE_ENHANCED_AVAILABLE = False
...
class YOLODataset(BaseDataset):
    ...
    def build_transforms(self, hyp: dict | None = None) -> Compose:
        ...
        # Enhanced Copy-Paste integration (only for segmentation task during training)
        if (
            self.augment
            and self.use_segments
            and hyp is not None
            and getattr(hyp, "copy_paste_enhanced", 0) > 0
            and _COPY_PASTE_ENHANCED_AVAILABLE
        ):
            try:
                # lazily build a singleton-like instance pool and cache on the dataset
                # 构建或加载实例池
                pool_file = getattr(hyp, "cp_pool_file", None)
                
                if pool_file and Path(pool_file).exists():
                    # 从文件加载自定义实例池
                    if not hasattr(self, "_cp_instance_pool") or self._cp_instance_pool is None or len(self._cp_instance_pool) == 0:
                        import pickle
                        LOGGER.info(f"Loading custom instance pool from: {pool_file}")
                        with open(pool_file, 'rb') as f:
                            self._cp_instance_pool = pickle.load(f)
                        LOGGER.info(f"Loaded {len(self._cp_instance_pool)} instances "
                                   f"({len(self._cp_instance_pool.egg_instances)} eggs, "
                                   f"{len(self._cp_instance_pool.barrier_instances)} barriers)")
                else:
                    # 自动构建实例池
                    if not hasattr(self, "_cp_instance_pool") or self._cp_instance_pool is None or len(self._cp_instance_pool) == 0:
                        pool_images = getattr(hyp, "cp_pool_images", 200)
                        pool_size = getattr(hyp, "cp_pool_size", 1000)
                        LOGGER.info(
                            f"Building Copy-Paste instance pool (images={pool_images}, max_size={pool_size})"
                        )
                        self._cp_instance_pool = build_instance_pool_from_dataset(
                            self, max_images=pool_images, max_pool_size=pool_size, sample_strategy='uniform'
                        )

                # 获取配置参数
                n_paste = getattr(hyp, "cp_n_paste", [1, 5])
                scale_range = getattr(hyp, "cp_scale_range", [0.5, 1.5])
                min_area = getattr(hyp, "cp_min_area", 100)
                blend = getattr(hyp, "cp_blend", False)
                blend_kernel = getattr(hyp, "cp_blend_kernel", 5)
                balance_classes = getattr(hyp, "cp_balance_classes", True)

                transforms.append(
                    CopyPasteEnhanced(
                        instance_pool=self._cp_instance_pool,
                        p=getattr(hyp, "copy_paste_enhanced", 0.5),
                        n_paste=(int(n_paste[0]), int(n_paste[1])) if isinstance(n_paste, (list, tuple)) else (1, 5),
                        scale_range=(float(scale_range[0]), float(scale_range[1]))
                        if isinstance(scale_range, (list, tuple))
                        else (0.5, 1.5),
                        min_area_threshold=int(min_area),
                        blend_edges=bool(blend),
                        blend_kernel=int(blend_kernel),
                        balance_classes=bool(balance_classes),
                    )
                )
                LOGGER.info(
                    f"Enhanced Copy-Paste enabled (p={getattr(hyp, 'copy_paste_enhanced', 0.5)}, "
                    f"n_paste={n_paste}, scale={scale_range}, balance={balance_classes})"
                )
            except Exception as e:  # pragma: no cover - robustness
                LOGGER.warning(f"Failed to enable Enhanced Copy-Paste: {e}")
